...
SELF_IS_RED = True # 己方是不是红方   蓝方 Done
...
class StateMachine:
    ...
    def __init__(self, node: Node):
        ...
        self.posture = 0                        # 哨兵姿态
        if SELF_IS_RED:
            self.self_cruising_list = [(19.93, 3.45),(19.54, 0.07)]  # 己方巡航列表[(10.02, -2.62),(8.14, -4.12)] 沈阳[x: 19.933201, y: 3.447407, z: 0.000000][x: 19.540752, y: 0.071469, z: 0.000000]
        else:
            self.self_cruising_list = [(0.63, -0.94),(1.99, 3.13)]#[(11.80,-1.42),(11.17,-2.04),(10.53,-3.34)] #[(0.63, -0.94),(1.99, 3.13)]  # 己方巡航列表 [(15.16, 1.61),(16.99, 1.37)] 沈阳[x: 0.633225, y: -0.939607, z: 0.000000][x: 1.986045, y: 3.129701, z: 0.000000]
        self.center_cruise_index = 0         # 中心巡航索引
        if SELF_IS_RED: 
            self.center_cruising_list = [(9.77, -1.78),(9.58, 4.18)]  # 中心巡航列表[(4.69, -3.18),(3.15, -2.60)] 沈阳[(9.77, -1.78),(9.58, 4.18)]
        else:
            self.center_cruising_list = [(8.67, 4.07),(8.82, -2.11)]  # 中心巡航列表[(16.24, 1.01),(15.35, 1.10)] 沈阳[x: 8.674704, y: 4.073686, z: 0.000000][x: 8.816567, y: -2.105113, z: 0.000000]
    ...
    def update(self, game_state, remaining_time, self_health,
        ...
        if(game_state == 3):
            ...
            # 推前哨站(敌方前哨站有血量)
            if self._handle_outpost_situation(enemy_outpost_hp):
                return
    ...
    def _handle_outpost_situation(self,enemy_outpost_hp):
        "推前哨站"
...
class Nav2DecisionNode(Node):
    ...
    def _declare_parameters(self) -> None:
        """声明所有参数"""
        if SELF_IS_RED:
            self.declare_parameter('home_position', [21.00, 6.25])  # 补血点  [-2.30, -3.65] 沈阳[x: 21.004612, y: 6.252463, z: 0.000000]
            self.declare_parameter('enemy_outpost_position', [9.75, -1.76])  # 敌方前哨站位置 11.13, 3.59 沈阳[x: 9.693395, y: -1.064331, z: 0.000000]  [x: 9.749234, y: -1.762673, z: 0.000000]
            self.declare_parameter('self_outpost_position', [7.68, -4.55])  # 己方前哨站(保护)
            self.declare_parameter('fortress_position', [2.69, 0.25])  # 堡垒点
        else:
            self.declare_parameter('home_position', [-1.78, -4.45])  # 补血点 [22.01, 2.50] 沈阳（-1.78,-4,45）
            self.declare_parameter('enemy_outpost_position', [9.78, 4.32])  # 敌方前哨站位置 [9.25, -4.74] 9.15, -4.93  6.86, -4.52 沈阳[x: 9.000474, y: 3.339411, z: 0.000000] [x: 9.785455, y: 4.322051, z: 0.000000]
            self.declare_parameter('self_outpost_position', [11.71, 3.13])  # 己方前哨站(保护) [11.71, 3.13]
            self.declare_parameter('fortress_position', [17.39, -1.35])  # 堡垒点[17.39, -1.35] [x: -1.782663, y: -4.445014, z: 0.000000]
    ...
    def _update(self) -> None:
        ...
        msg = UInt8()
        msg.data = self.state_machine.posture
