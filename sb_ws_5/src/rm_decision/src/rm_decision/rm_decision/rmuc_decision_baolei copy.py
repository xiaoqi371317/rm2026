...
SELF_IS_RED = True # 己方是不是红方   蓝方 TODO
...
class StateMachine:
    ...
    def __init__(self, node: Node):
        ...
        if SELF_IS_RED:
            self.self_cruising_list = [(4.53, -2.52),(2.78, -2.40)]  # 己方巡航列表
        else:
            self.self_cruising_list = [(15.16, 1.61),(16.99, 1.37)]  # 己方巡航列表
        self.baolei_cruise_index = 0                   # 堡垒巡航索引
        if SELF_IS_RED:
            self.baolei_cruising_list = [(3.94, 1.81),(2.66, 0.31),(2.60, -1.79),(2.81, 0.28)]  # 堡垒巡航列表
        else:
            self.baolei_cruising_list = [(17.51, -1.39),(16.04, 1.01),(17.42, -1.39),(16.73, -2.95)]  # 堡垒巡航列表
        self.center_cruise_index = 0         # 中心巡航索引
        if SELF_IS_RED:
            self.center_cruising_list = [(11.82, -4.06),(11.50, 2.02),(8.29, 2.21),(7.30, -2.24)]  # 中心巡航列表
        else:
            self.center_cruising_list = [(8.64, 2.21),(11.48, 2.14),(10.43, -3.75),(7.62, -2.09)]  # 中心巡航列表
        
        self.stay_timer = None
        self.self_stay_timer = None
        self.baolei_stay_timer = None

        self.stay_duration = 12        # 巡航停留时间
        self.self_stay_duration = 5       # 巡航停留时间
        self.baolei_stay_duration = 12       # 堡垒巡航停留时间
    ...
    def update(self, game_state, remaining_time, self_health,
        ...
        self.at_castle_area = at_castle_area
        ...
        if(game_state == 3):
            ...
            # 上堡垒
            if self._handle_fortress_situation(remaining_time, self_base_hp, at_castle_area):
                return
            
            # TODO 己方半场巡逻（保守策略）目前只有一个点生效
    ...
    # TODO
    def _handle_fortress_situation(self,remaining_time, self_base_hp, at_castle_area):
        "上堡垒"
        if(remaining_time < 120 or self_base_hp < 3000):
            if(self.current_state != RobotState.GOING_FORTRESS):
                self._start_baolei_cruise()
                self._start_baolei_stay_timer()
                return True
    ...
    # 堡垒测试！！！！！！！！！！！！！！！！！！！！！！
    def _start_baolei_cruise(self) -> None:
        """开始堡垒巡航模式"""
        self.baolei_cruise_index = self.baolei_cruise_index % len(self.baolei_cruising_list)
        self._transition_to(RobotState.GOING_FORTRESS, self.baolei_cruising_list[self.baolei_cruise_index])

    def _start_baolei_stay_timer(self) -> None:
        """启动停留定时器"""
        if self.baolei_stay_timer:
            self.baolei_stay_timer.cancel()
        self.baolei_stay_timer = self.node.create_timer(self.baolei_stay_duration, self._handle_baolei_stay_complete)

    def _handle_baolei_stay_complete(self) -> None:
        """处理堡垒巡航停留结束"""
        if self.current_state == RobotState.GOING_FORTRESS:
            if self.at_castle_area != 1:
                self.baolei_cruise_index = (self.baolei_cruise_index + 1) % len(self.baolei_cruising_list)
                self._transition_to(RobotState.GOING_FORTRESS, self.baolei_cruising_list[self.baolei_cruise_index])
            else:
                self.baolei_cruise_index = self.baolei_cruise_index % len(self.baolei_cruising_list)
                self._transition_to(RobotState.GOING_FORTRESS, self.baolei_cruising_list[self.baolei_cruise_index])
                self.node.get_logger().info(f"已经登上堡垒")
...
class Nav2DecisionNode(Node):
    ...
    def _declare_parameters(self) -> None:
        """声明所有参数"""
        if SELF_IS_RED:
            self.declare_parameter('home_position', [-2.30, -3.57])  # 补血点
            self.declare_parameter('enemy_outpost_position', [11.79, 3.36])  # 敌方前哨站位置
