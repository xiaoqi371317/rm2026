...
RED_TARGETS: Dict[str, Target] = {
    ...
    'home_side_patrol_0': Target(10.02, -2.62),
    'home_side_patrol_1': Target(8.14, -4.12),
}
...
BLUE_TARGETS: Dict[str, Target] = {
    ...
    'home_side_patrol_0': Target(15.16, 1.61),
    'home_side_patrol_1': Target(16.99, 1.37),
}
...
class CenterFirstDecisionNode(Node):
    def __init__(self):
        super().__init__('qianshao_center_first_decision_node')
        ...
        self.opening_center_patrol_sec = float(
            self.declare_parameter('opening_center_patrol_sec', 45.0).value
        )
        ...
        self.home_side_patrol_targets = ['home_side_patrol_0', 'home_side_patrol_1']
        ...
        self.get_logger().info(
            f'Center-first qianshao decision started. action_server={self.action_server_name}, '
    ...
    def on_navigation_result(self, target_name: Optional[str], status: int) -> None:
        ...
        if (
            self.current_state == RobotState.HOME_SIDE_PATROL
            and target_name in self.home_side_patrol_targets
        ):
            self.patrol_hold_until = now + self.patrol_hold_sec
            return
    ...
    def _update(self) -> None:
        ...
        elif self.current_state == RobotState.HOME_SIDE_PATROL:
            self._handle_home_side_patrol()
    ...
    def _handle_return_home(self, msg: RefereeData) -> None:
        ...
        resume_state = self.previous_state_before_return or RobotState.CENTER_PATROL
        self.return_due_to_health = False
        self.return_due_to_bullet = False
    ...
    def _handle_center_patrol(self) -> None:
        if self.match_start_time is not None:
            ...
            if elapsed >= self.opening_center_patrol_sec:
    ...
    def _handle_attack_outpost(self) -> None:
        ...
        if self.outpost_arrival_time is not None:
            if now - self.outpost_arrival_time >= self.outpost_attack_dwell_sec:
                ...
                self._transition_to(RobotState.HOME_SIDE_PATROL)
            return

        if now - self.outpost_phase_start_time >= self.outpost_attack_timeout_sec:
            self.get_logger().warning('Outpost attack timeout; entering home-side patrol')
            self._transition_to(RobotState.HOME_SIDE_PATROL)
            return

        self._send_named_target('enemy_outpost', POSTURE_ATTACK)

    def _handle_home_side_patrol(self) -> None:
        self._run_patrol(self.home_side_patrol_targets, 'home_side_index')
    ...
    def _run_patrol(self, target_names: List[str], index_attr: str) -> None:
        ...
        self._send_named_target(target_name, POSTURE_DEFENSE)
