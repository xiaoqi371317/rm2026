...
Main behavior:
  ...
  2. Recovery resumes the interrupted home patrol phase.
...
class SmartQianshaoDecisionNode(Node):
    ...
    def _publish_debug_state(self) -> None:
        ...
        if msg is None:
            debug.data = (
                ...
                f'time_source={time_source}, fire_age={fire_age:.1f}, recent_fire={recent_fire}'
            )
        else:
            debug.data = (
    ...
    def _patrol_phase_debug(self) -> Tuple[str, float, float]:
        if self.current_state == RobotState.CENTER_PATROL:
            phase_name = 'center'
            phase_limit = self.center_patrol_phase_sec
