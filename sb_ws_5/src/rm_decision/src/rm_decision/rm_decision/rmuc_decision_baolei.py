...
class StateMachine:
    ...
    def __init__(self, node: Node):
        ...
        self.other_cruise_index = 0             # 敌方巡航索引
        if SELF_IS_RED:
            self.self_cruising_list = [(15.45, -5.92)]  # 己方巡航列表
            # self.center_cruising_list = [(10.84, 3.42),(8.35, 2.27),(8.80, -2.99),(11.44, -1.87)]  # 中心巡航列表
            # self.center_cruising_list = [(2.25, -6.73)]  # 中心巡航列表  13.98, -4.78
            self.center_cruising_list = [(13.98, -4.78)]  # 中心巡航列表  13.98, -4.78
            self.other_cruising_list = [(3.81, -2.72),(4.40, 1.03)]  # 敌方巡航列表
        else:
            self.self_cruising_list = [(4.35, 5.97)]  # 己方巡航列表
            # self.center_cruising_list = [(9.35, -3.60),(11.78, -1.38), (10.34, 3.16), (7.62, -0.68)]  # 中心巡航列表
            self.center_cruising_list = [(5.99, 4.93)]  # 中心巡航列表
            self.other_cruising_list = [(15.53, 2.47),(15.54, -1.52)]  # 敌方巡航列表
            
        self.stay_timer = None
        self.self_stay_timer = None
        self.other_stay_timer = None

        self.stay_duration = 8        # 中心巡航停留时间
        self.self_stay_duration = 5       # 巡航停留时间
        self.other_stay_duration = 5       # 敌方巡航停留时间
    ...
    def update(self, game_state, remaining_time, self_health,
        ...
        if(game_state == 3):
            ...
            # 对面半场巡航（和你爆了）
            if self._hander_other_cruise(remaining_time):
                return
    ...
    def _emergency_check_remaining_bullet(self, remaining_bullet) -> bool:
        """回家补弹"""
        if remaining_bullet <= 20 and self.current_state != RobotState.GOING_TO_HOME:
    ...
    def _hander_self_cruise(self,remaining_time):
        "己方半场巡航"
        if(remaining_time < 320 and remaining_time > 150):
    ...
    def _hander_other_cruise(self,remaining_time):
        '''对面半场巡航'''
        if(remaining_time < 150):
            if(self.current_state != RobotState.OTHER_CRUISING):
                self._start_other_cruise()
                self._start_other_stay_timer()
                return True
            elif self.current_state == RobotState.OTHER_CRUISING:
    ...
    ##########################  对面巡航 ######################################
    def _start_other_cruise(self) -> None:
        """开始敌方巡航模式"""
        self.other_cruise_index = self.other_cruise_index % len(self.other_cruising_list)
        self._transition_to(RobotState.OTHER_CRUISING, self.other_cruising_list[self.other_cruise_index])
    def _start_other_stay_timer(self) -> None:
        """启动敌方停留定时器"""
        if self.other_stay_timer:
            self.other_stay_timer.cancel()
        self.other_stay_timer = self.node.create_timer(self.other_stay_duration, self._handle_other_stay_complete)
    def _handle_other_stay_complete(self) -> None:
        """处理敌方巡航停留结束"""
        if self.current_state == RobotState.OTHER_CRUISING:
            self.other_cruise_index = (self.other_cruise_index + 1) % len(self.other_cruising_list)
            self._transition_to(RobotState.OTHER_CRUISING, self.other_cruising_list[self.other_cruise_index])
    ##########################  对面巡航 ######################################
...
class Nav2DecisionNode(Node):
    ...
    def _declare_parameters(self) -> None:
        """声明所有参数"""
        if SELF_IS_RED:
            self.declare_parameter('home_position', [21.35, 4.98])  # 补血点  -1.95, -3.60
            self.declare_parameter('enemy_outpost_position', [11.79, 3.36])  # 敌方前哨站位置(弃用)
            self.declare_parameter('self_outpost_position', [7.68, -4.55])  # 己方前哨站(弃用)
            self.declare_parameter('fortress_position', [2.69, 0.25])  # 堡垒点(弃用)
        else:
            self.declare_parameter('home_position', [-1.21, -5.05])  # 补血点
            self.declare_parameter('enemy_outpost_position', [9.25, -4.74])  # (弃用)
            self.declare_parameter('self_outpost_position', [11.71, 3.13])  # (弃用)
            self.declare_parameter('fortress_position', [17.39, -1.35])  # 堡垒点 (弃用)
