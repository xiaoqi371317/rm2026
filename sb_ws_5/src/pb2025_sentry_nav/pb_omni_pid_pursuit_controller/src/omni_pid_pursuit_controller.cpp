  ...
  std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".rotation_reference", rclcpp::ParameterValue("carrot_orientation"));
  declare_parameter_if_not_declared(
  ...
  node->get_parameter(plugin_name_ + ".rotation_reference", rotation_reference_);
  if (
    rotation_reference_ != "carrot_orientation" && rotation_reference_ != "path_heading" &&
    rotation_reference_ != "goal_orientation") {
    RCLCPP_WARN(
      logger_,
      "Unknown rotation_reference '%s'. Falling back to 'carrot_orientation'.",
      rotation_reference_.c_str());
    rotation_reference_ = "carrot_orientation";
  }
  ...
  nav2_core::GoalChecker * /*goal_checker*/)
{
  ...
  double rotation_error = getRotationError(transformed_plan, carrot_pose, theta_dist);

  if (use_rotate_to_heading_) {
    rotation_error = tf2::getYaw(transformed_plan.poses.back().pose.orientation);
    if (fabs(rotation_error) > use_rotate_to_heading_treshold_) {
      lin_dist = 0;
    }
  }

  auto lin_vel = move_pid_->calculate(lin_dist, 0);
  auto angular_vel = enable_rotation_ ? heading_pid_->calculate(rotation_error, 0) : 0.0;
...
double OmniPidPursuitController::getRotationError(
  const nav_msgs::msg::Path & transformed_plan,
  const geometry_msgs::msg::PoseStamped & carrot_pose, double path_heading) const
{
  if (rotation_reference_ == "path_heading") {
    return path_heading;
  }

  if (rotation_reference_ == "goal_orientation") {
    return tf2::getYaw(transformed_plan.poses.back().pose.orientation);
  }

  return tf2::getYaw(carrot_pose.pose.orientation);
}
  ...
  std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (const auto & parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      ...
      if (name == plugin_name_ + ".rotation_reference") {
        auto rotation_reference = parameter.as_string();
        if (
          rotation_reference == "carrot_orientation" || rotation_reference == "path_heading" ||
          rotation_reference == "goal_orientation") {
          rotation_reference_ = rotation_reference;
        } else {
          result.successful = false;
          result.reason =
            "rotation_reference must be carrot_orientation, path_heading, or goal_orientation";
          return result;
        }
      }
    }
