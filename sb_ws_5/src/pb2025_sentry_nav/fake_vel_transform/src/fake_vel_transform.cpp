...
constexpr double CONTROLLER_TIMEOUT = 2.5;
constexpr double ZERO_CMD_VEL_DURATION = 0.8;
constexpr double FOLLOW_YAW_ANGLE = 0.7853981633974483;
constexpr const char * FOLLOW_PATH_YAW_CONTROLLER = "FollowPathYaw";
constexpr const char * FOLLOW_PATH_SPIN_CONTROLLER = "FollowPathSpin";
...
: Node("fake_vel_transform", options)
{
  ...
  this->declare_parameter<bool>("if_follow_yaw", false);
  ...
  this->get_parameter("controller_selector_topic", controller_selector_topic_);
  this->get_parameter("init_spin_speed", spin_speed_);
  bool if_follow_yaw = false;
  this->get_parameter("if_follow_yaw", if_follow_yaw);
  if_follow_yaw_.store(if_follow_yaw);

  tf_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*this);

  cmd_vel_chassis_pub_ =
    this->create_publisher<geometry_msgs::msg::Twist>(output_cmd_vel_topic_, 1);
  controller_selector_pub_ = this->create_publisher<std_msgs::msg::String>(
    controller_selector_topic_, rclcpp::QoS(1).transient_local().reliable());
  publishControllerSelector(if_follow_yaw_.load());
  ...
  if_follow_yaw_sub_ = this->create_subscription<std_msgs::msg::Bool>(
    "if_follow_yaw", 1,
    std::bind(&FakeVelTransform::followYawCallback, this, std::placeholders::_1));
...
void FakeVelTransform::followYawCallback(std_msgs::msg::Bool::SharedPtr msg)
{
  //if_follow_yaw_.store(msg->data);
  const bool previous_if_follow_yaw = if_follow_yaw_.exchange(msg->data);
  if (previous_if_follow_yaw == msg->data) {
    return;
  }
  ...
  zero_cmd_vel_until_sec_.store(rclcpp::Clock().now().seconds() + ZERO_CMD_VEL_DURATION);
  publishZeroVelocity();
  publishControllerSelector(msg->data);
}

void FakeVelTransform::publishControllerSelector(bool if_follow_yaw)
{
  std_msgs::msg::String msg;
  msg.data = if_follow_yaw ? FOLLOW_PATH_YAW_CONTROLLER : FOLLOW_PATH_SPIN_CONTROLLER;
  controller_selector_pub_->publish(msg);
}

void FakeVelTransform::publishZeroVelocity()
{
  cmd_vel_chassis_pub_->publish(geometry_msgs::msg::Twist());
}

bool FakeVelTransform::zeroVelocityHoldActive() const
{
  return rclcpp::Clock().now().seconds() < zero_cmd_vel_until_sec_.load();
}
...
void FakeVelTransform::publishTransform()
{
  ...
  tf2::Quaternion q;
  if (if_follow_yaw_.load()) {
    q.setRPY(0, 0, FOLLOW_YAW_ANGLE);
  } else {
  ...
  const geometry_msgs::msg::Twist::SharedPtr & twist, float yaw_diff)
{
  ...
  if (if_follow_yaw_.load()) {
    const double cos_angle = cos(FOLLOW_YAW_ANGLE);
    const double sin_angle = sin(FOLLOW_YAW_ANGLE);
    aft_tf_vel.angular.z = twist->angular.z;
    aft_tf_vel.linear.x = twist->linear.x * cos_angle - twist->linear.y * sin_angle;
    aft_tf_vel.linear.y = twist->linear.x * sin_angle + twist->linear.y * cos_angle;
  } else {
